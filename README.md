...
Он парсит конфишурацию и создаюет:
...
P.S. видимо название метода: путь к энпоинту + имя метода, например `/api/users/getall` -> `APIUsersGetAllGet` и `APIUsersGetAllGetParams/Res` если в operationId не указано обратное
...
Я исполбзовую res-структуры, можно включить convenient errors, но тогда управление ставновится менее очевидным:
Во первых мне придется делать обертку над api.Error чтобы она возвращада JSON строку на Error(), что не очень идиоматично,
Во вторых тогда если возвращать ошибку из метода то будет не ошибка 500, а моя кастомная ошибка, которая не устанавливает в зоголовок нужный код в заголовок, что опять лишний код в middlewares:
